...
test_that("plot.ipmr_uncertainty runs", {

  ipm <- make_test_ipm("simple_di_det")

  samples <- as.data.frame(matrix(
    rep(unlist(parameters(ipm)), each = 5),
    nrow = 5,
    byrow = TRUE
  ))
  ...
  expect_s3_class(plot(res, type = "stacked"), "ggplot")
})

test_that("plot type 'all' returns list", {
  ...
  expect_true(all(c("param", "vr", "stacked") %in% names(out)))
})

test_that("plot errors on unnamed vr_colors", {
...
test_that("plot errors on missing vr_colors entries", {
...
test_that("print.ipmr_uncertainty runs", {
